...
**Requisito acrescentado em 2026-09-10, por recomendação do revisor, e reduzido em 2026-09-24:** desempatar parcela × data usando o `occurredOn` da própria linha — **quando ele existe**. O caso do `N/M` como *único* candidato a data passou para o `text.ts` (T-119), porque ali o `occurredOn` ainda não existe e o desempate do pipeline seria inalcançável. Ver a nota em `CONTRACTS.md` §15. Um par `N/M` que coincide com o dia e o mês da transação — descrição `03/10` numa linha de `2026-10-03` — é data, não parcela. O `detectInstallment` (T-121) só recebe a descrição e não tem como saber; o pipeline tem a informação e custa uma comparação. Sem isso, cerca de 1 em 5 datas `dd/mm` viram parcela fantasma projetada por M meses.
