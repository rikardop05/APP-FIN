...
| `portal screenshot` falha com *not rendering* **só nas capturas de 390px**, com a janela comprovadamente restaurada | **`maestri portal resize` derruba a renderização.** O print em tamanho de desktop sai; o resize para 390×844 retorna viewport correta, mas o screenshot seguinte falha. Isolado pelo Funil em 2026-09-24, ao notar que o desktop funcionou e todos os de mobile falharam **depois** do resize | Capturar o desktop primeiro. Para mobile, se falhar, reportar como **não verificado** — e **nunca** recortar o print de desktop: o layout responsivo muda por *media query*, então um recorte de 1900 mostra o layout de desktop numa moldura estreita, que é exatamente o que não se quer verificar |
